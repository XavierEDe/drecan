...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? 'bg-forest-900/98 backdrop-blur-md shadow-xl py-2' : 'bg-forest-900/90 backdrop-blur-sm py-4'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
        <Link to="/#" className="flex items-center gap-3 group" onClick={() => handleNavClick('hero')}>
          <img src={logo} alt="Drecan Agriculture Logo" className="h-10 w-10 object-contain" />
          <div className="hidden sm:block">
            <div className="font-display font-lg text-white text-bg leading-tight">DRECAN COMMODITIES</div>
            <div className="font-body text-gold-400 text-xs font-semibold tracking-wider uppercase">Feed The Nation</div>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          <a
            href="https://wa.me/2349021649580"
            target="_blank"
